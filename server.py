...
# vLLM integration: use the logits processor
from lmformatenforcer.integrations.vllm import build_vllm_logits_processor
# Build tokenizer_data from a HF tokenizer (works with awaited AsyncLLMEngine.get_tokenizer())
...
# HF tokenizer (for chat template convenience)
...
try:
    ...
    raise
# =========================
# Tokenizer (synchronous, for chat template convenience)
# =========================
try:
    ...
    raise
# Build tokenizer_data for logits processor later (requires awaiting AsyncLLMEngine.get_tokenizer())
tokenizer_data = None
@app.on_event("startup")
async def _startup_build_tokenizer_data():
    global tokenizer_data
    try:
        # Await the async tokenizer getter
        vllm_tokenizer = await llm.get_tokenizer()
        tokenizer_data = build_token_enforcer_tokenizer_data(vllm_tokenizer)
        print("Tokenizer data for format enforcement initialized.")
    except Exception as e:
        print(f"CRITICAL: Failed to build tokenizer data for enforcer: {e}")
        raise
# =========================
...
# UPDATED: Robust field checking compatible with Pydantic, Dataclasses, and msgspec.Struct
def _sp_has_field(field: str) -> bool:
    # Robust field existence check across vLLM versions
    try:
        # Pydantic V2
        if hasattr(SamplingParams, "model_fields"):
            return field in getattr(SamplingParams, "model_fields", {})
        # Dataclass / Pydantic V1
        if hasattr(SamplingParams, "__dataclass_fields__"):
            return field in getattr(SamplingParams, "__dataclass_fields__", {})
        # msgspec.Struct (used in recent vLLM versions)
        if hasattr(SamplingParams, "__struct_fields__"):
            return field in getattr(SamplingParams, "__struct_fields__", ())
        # General annotation check (Fallback for other structured classes)
        if hasattr(SamplingParams, "__annotations__"):
            return field in getattr(SamplingParams, "__annotations__", {})
        # CRITICAL CHANGE: We must NOT use a generic `hasattr(SamplingParams, field)`
        # as it can incorrectly match methods or properties not accepted by the constructor.
        return False
    except Exception:
        return False
def _ensure_tokenizer_data_ready():
    if tokenizer_data is None:
        raise HTTPException(
            status_code=500,
            detail="Tokenizer data not initialized yet. Wait for server startup to complete.",
        )
def _build_logits_processors(vocab_lang, vocab_n_words):
    if not (vocab_lang and vocab_n_words):
        return None, None
    _ensure_tokenizer_data_ready()
...
def _create_sampling_params(request) -> SamplingParams:
    ...
    _, logits_processors = _build_logits_processors(
        params["vocab_lang"], params["vocab_n_words"]
    )
    # Beam search handling
    use_beam_search = params["num_beams"] > 1
    if use_beam_search:
        # Deterministic settings for beam search
        params["temperature"] = 0.0
        params["top_p"] = 1.0
        # n (number of returned sequences) cannot exceed the number of beams (beam width)
        params["n"] = min(max(1, int(params.get("n", 1))), params["num_beams"])
    ...
    # Optional parameters (guarded by field presence)
    ...
    # UPDATED: Modernized Beam Search Logic using best_of
    # Beam vs sampling specific knobs
    if use_beam_search:
        # In modern vLLM, beam search is activated by setting best_of > 1 and temperature = 0.
        # Temperature is already set to 0.0 earlier in this function.
        # We set best_of to the number of beams requested (the beam width).
        sp_kwargs["best_of"] = params["num_beams"]
    else:
        # Sampling mode (temperature > 0 or num_beams=1).
        # best_of determines how many sequences to generate in parallel.
        # It usually defaults to n, but we set it explicitly for clarity.
        sp_kwargs["best_of"] = max(1, params["n"])
    try:
        sampling_params = SamplingParams(**sp_kwargs)
    except (ValueError, TypeError) as e:
        # Improved error logging
        print(f"SamplingParams error: {e}. Kwargs: {sp_kwargs}")
...
async def chat_completions(
    ...
    try:
        ...
        # Re-raise HTTP exceptions that are already handled (e.g. 400 errors)
        raise
    except Exception as e:
        # Catch unexpected runtime errors during sampling param creation
        print(f"Runtime error creating SamplingParams: {e}")
        raise HTTPException(status_code=500, detail=f"Internal error preparing generation: {e}")
